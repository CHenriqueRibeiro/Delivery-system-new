...
export default function Menu() {
  ...
  const [opcionaisSelecionados, setOpcionaisSelecionados] =
    useState();
    ...
    const fetchTabData = () => {
      fetch(
        ...
        .then((data) => {
          ...
          if (activeTab === 'combos') {
            objGenerico = data.adicionais.combos;
            optionalList = data.opcionais.combos;
          } else if (activeTab === 'pizza') {
            objGenerico = data.adicionais.pizza;
            optionalList = data.opcionais.pizza;
          } else if (activeTab === 'hamburguer') {
            objGenerico = data.adicionais.hamburguer;
            optionalList = data.opcionais.hamburguer;
          } else if (activeTab === 'paoArabe') {
            objGenerico = data.adicionais.paoArabe;
            optionalList = data.opcionais.paoArabe;
          }
  ...
  /* o problema ta aqui, nao ta pegando o valor de cada opcional*/

  const itemOptionalSelected =
    firebaseData &&
    firebaseData.opcionais &&
    firebaseData.opcionais[activeTab]
      ? firebaseData.opcionais[activeTab].find(
          (opcional) => opcional.opcao === opcionais
        )
      : null;
  // console.log('Valor Selecionado:', itemOptionalSelected);
  const valorAdcOpcaoEscolhida = itemOptionalSelected
    ? itemOptionalSelected.valorAdc
    : 0;
  // console.log(
  //   'valor atual do opcional:',
  //   itemOptionalSelected?.valorAdc
  // );
      ...
      >
        ...
        >
          ...
          <RadioGroup
            ...
            aria-label="borda"
            name="borda"
            value={opcionaisSelecionados}
            onChange={(e) => {
              const selectedValue = e.target.value;
              const selectedOption = opcionais.find(
                (item) => item.opcao === selectedValue
              ); // Encontre o objeto correspondente
              if (selectedOption) {
                setOpcionaisSelecionados(selectedValue);
                console.log(
                  'opcional selecionado:',
                  selectedValue
                );
                console.log(
                  'Valor Adc selecionado:',
                  selectedOption.valorAdc
                );
                setRefrigeranteError('');
              }
            }}
